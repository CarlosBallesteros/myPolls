...
export default class SelectPoll extends Component {
  ...
  render() {
    ...
    const visiblePolls = polls.filter( poll => poll.isHidden === false);
    const noResults = visiblePolls.length === 0 ? 'No results' : null;
    return (
      <div className="row">
        <div className="col-md-6">
          ...
          { this.state.loading ? <Spinner /> : (<div>
            <ul className="list-group">
              {
                visiblePolls.map( (poll, index) =>  <li className="list-group-item" key={index}><Link to={`/vote/${poll.id}`}>{poll.title}</Link></li> )
              }
           </ul>
           <h4>{ noResults }</h4>
          </div>)}
